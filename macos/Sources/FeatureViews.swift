                ...
                VStack(spacing: 12) {
                    ...
                    HStack {
                        ...
                        if !model.supportsWrites {
                            Text("执行与排序仅 API Key 模式可用。")
                                .font(.caption).foregroundStyle(.orange)
                        }
                        Spacer()
                        Button("恢复基线排序") { model.applyLoadedOrdering() }
                            .disabled(!plan.synchronizeDeletes || !hasOrdering || !model.canWrite)
                            .help("仅 API Key 模式可恢复排序")
                        Button("执行所选变更") {
                            if plan.selectedDeleteCount > 0 { confirmingExecution = true }
                            else { model.executeSelectedPlanChanges() }
                        }
                        .buttonStyle(.borderedProminent)
                        .disabled(plan.selectedCount == 0 || !model.canWrite)
                        .help("仅 API Key 模式可执行变更")
                    }
                }
                .padding(16)
            } else {
